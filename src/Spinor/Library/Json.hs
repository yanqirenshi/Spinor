...
spinorToAeson (VList xs)
  ...
  where
    ...
    isPair (VList [VSym s, _]) = ":" `T.isPrefixOf` s  -- キーワードシンボル
    isPair _                   = False
    ...
    toPair (VList [VSym s, v]) | ":" `T.isPrefixOf` s = do
      v' <- spinorToAeson v
      Right (AK.fromText (T.drop 1 s), v')  -- ":" を除去してキーとする
    toPair _ = Left "json-stringify: Alist の要素は (:key value) または (\"key\" value) の形式である必要があります"
...
spinorToAeson (VSocket _)     = Left "json-stringify: Socket は JSON に変換できません"
...
aesonToSpinor (Aeson.Object obj) =
  -- JSON オブジェクトを Alist に変換 (キーワードシンボル使用)
  -- { "a": 1, "b": 2 } -> ((:a 1) (:b 2))
  VList (map toPair (AKM.toList obj))
  where
    toPair (k, v) = VList [VSym (":" <> AK.toText k), aesonToSpinor v]
