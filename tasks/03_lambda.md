...
`fn` (lambda) 構文とクロージャを実装し、「関数を定義し適用できる言語」にする。`Val` と `Env` の相互参照（循環依存）問題を解決し、レキシカルスコープを実現する。

## 設計判断

### 循環参照の解決

`VFunc` が `Env` を保持し、`Env` が `Val` を保持するため循環参照が発生する。
解決策: `Val` と `Env` の両方を `Val.hs` に同居させ、`Eval.hs` からインポートする。

### VFunc コンストラクタ

`VFunc [Text] Expr Env` — 引数名リスト、関数本体の式、定義時の環境スナップショットを保持。
定義時の環境をキャプチャすることでレキシカルスコープ（クロージャ）を実現する。

### apply 関数の設計

`VPrim` と `VFunc` をパターンマッチで分岐:
- `VPrim`: 従来通り純粋関数を適用
- `VFunc`: クロージャ環境 + 引数束縛で新環境を構築 → body を評価 → 元の環境を復元

環境復元により、関数適用がレキシカルスコープで動作する。

## 変更の流れ

1. `src/Spinor/Val.hs` (修正) — `Env` を `Eval.hs` から移動、`VFunc` コンストラクタ追加
2. `src/Spinor/Eval.hs` (修正) — `fn` 特殊形式の eval、`apply` に `VFunc` 対応追加
3. `src/Spinor/Primitive.hs` (修正) — `Env` のインポート元を変更
4. `app/Main.hs` (修正) — `Env` のインポート元変更
